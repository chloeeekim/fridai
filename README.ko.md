...
| `FRIDAI_FASTEMBED_MODEL` | `nomic-ai/nomic-embed-text-v1.5` | fastembed 모델명. 바꾸려면 `fridai index --reindex --source all`로 전체 재인덱싱 필요; 인덱싱과 쿼리(MCP 서버 포함)에서 동일하게 설정하지 않으면 recall이 어휘 검색으로 폴백. |
...
**비영어 콘텐츠:** 기본값 `nomic-ai/nomic-embed-text-v1.5`는 영어 중심이라 한국어 등 다른 언어의
시맨틱 recall 품질이 떨어집니다. 다국어 코드/대화라면 `FRIDAI_FASTEMBED_MODEL`을 다국어 모델로
지정하고 재인덱싱하세요 — 예: `sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2`
(384차원, 작고 빠르며 두루 무난) 또는 `intfloat/multilingual-e5-large`(1024차원, 품질↑ 무거움).
바꾼 뒤 `fridai index --reindex --source all` 실행.
